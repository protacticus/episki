...
class CheckRequirementsSubscriber implements EventSubscriberInterface
{
    ...
    public function handleConsoleError(ConsoleErrorEvent $event)
    {
        ...
        if ($event->getCommand() && in_array($event->getCommand()->getName(), $commandNames, true)) {
            if ($this->isSQLitePlatform() && !extension_loaded('sqlite3')) {
                ...
                $io->error('This command requires to have the "sqlite3" PHP extension enabled because, by default, the episki application uses SQLite to store its information.');
            }
        }
    }
    ...
    public function handleKernelException(GetResponseForExceptionEvent $event)
    {
        ...
        if ($isDriverException && $this->isSQLitePlatform() && !extension_loaded('sqlite3')) {
            $event->setException(new \Exception('PHP extension "sqlite3" must be enabled because, by default, the episki application uses SQLite to store its information.'));
        }
    }
